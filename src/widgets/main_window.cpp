...
void MainWindow::treeviewCurrentItemChanged(const QModelIndex& current, const QModelIndex& previous)
{
    Q_UNUSED(current);
    Q_UNUSED(previous);
    updateActions();
...
{
    QString directory =
        QFileDialog::getExistingDirectory(this, tr("Open Folder"), m_currentDirectory);
...
{
    QString mcDirectory =
        QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/../.minecraft/saves";
...
void MainWindow::saveAs()
{
    ...
    QModelIndex index = m_ui->nbtDataTreeView->currentIndex();
    if(index.isValid()) {
        NbtTreeItemNbtFile* treeItem =
            dynamic_cast<NbtTreeItemNbtFile*>(m_nbtTreeModel->fromIndex(index));
        if(!treeItem) {
            return;
        }
        ...
        QString path            = treeItem->path();
        QString filename =
            QFileDialog::getSaveFileName(this, tr("Save %1 as...").arg(currentFilename), path);
...
void MainWindow::refresh()
{
    ...
    if(index.isValid()) {
        if(m_nbtTreeModel->isDirty(index)) {
            ...
            btn = QMessageBox::warning(this, tr("%1 - Unsaved Changes").arg(m_baseWindowTitle),
...
{
    /// General UI
    connect(m_ui->nbtDataTreeView->selectionModel(), &QItemSelectionModel::currentChanged, this,
            &MainWindow::treeviewCurrentItemChanged);
    connect(m_ui->nbtDataTreeView, &QTreeView::customContextMenuRequested, this,
            &MainWindow::showCustomContextMenu);
    connect(m_ui->nbtDataTreeView, &QTreeView::doubleClicked, this, &MainWindow::editTag);
    connect(this->m_nbtTreeModel, &NbtDataTreeModel::modified, this, &MainWindow::modifiedModel);
    ...
    connect(m_ui->actionOpen_Minecraft_Save_Folder, &QAction::triggered, this,
            &MainWindow::openMinecraftFolder);
...
bool MainWindow::userReallyWantsToQuit()
{
    if(m_nbtTreeModel->isModified()) {
        QMessageBox::StandardButton btn;
        btn = QMessageBox::warning(this, m_baseWindowTitle,
                                   tr("The application has unsaved changes.\n"
